...
// =============================================================================
// StringSet Tests (legacy type)
// =============================================================================

func TestStringSetBasics(t *testing.T) {
	s := NewStringSet("a", "b", "c")
	if len(s) != 3 {
		t.Errorf("StringSet: expected 3 elements, got %d", len(s))
	}

	s.Add("d")
	if !s.Contains("d") {
		t.Error("StringSet Add: value not added")
	}

	s.Remove("a")
	if s.Contains("a") {
		t.Error("StringSet Remove: value not removed")
	}
}

func TestStringSetValuesSorted(t *testing.T) {
	...
	for i, v := range vals {
		if v != expected[i] {
			t.Errorf("StringSet Values: expected sorted %v, got %v", expected, vals)
			break
		}
	}
}
...
func TestStringSetIntersection(t *testing.T) {
	...
	if len(inter) != 2 {
		t.Errorf("StringSet Intersection: expected 2 elements, got %d", len(inter))
	}
	if !inter.Contains("b") || !inter.Contains("c") {
		t.Error("StringSet Intersection: wrong values")
	}
}

// =============================================================================
// IntSet Tests (legacy type)
// =============================================================================

func TestIntSetBasics(t *testing.T) {
	s := NewIntSet(1, 2, 3)
	if len(s) != 3 {
		t.Errorf("IntSet: expected 3 elements, got %d", len(s))
	}

	s.Add(4, 5)
	if len(s) != 5 {
		t.Errorf("IntSet Add: expected 5 elements, got %d", len(s))
	}

	s.Remove(1)
	if s.Contains(1) {
		t.Error("IntSet Remove: value not removed")
	}
}

func TestIntSetValuesSorted(t *testing.T) {
	s := NewIntSet(3, 1, 2)
	vals := s.Values()
	...
	for i, v := range vals {
		if v != expected[i] {
			t.Errorf("IntSet Values: expected sorted %v, got %v", expected, vals)
			break
		}
	}
}
...
func TestIntSetSubtract(t *testing.T) {
	...
	if len(diff) != 2 {
		t.Errorf("IntSet Subtract: expected 2 elements, got %d", len(diff))
	}
}
